...
DW$3	.dwtag  DW_TAG_subprogram, DW_AT_name("CONTROL_ISR"), DW_AT_symbol_name("_CONTROL_ISR")
...
;	C:\step_tracer\a_3rd_race\Compiler\bin\opt2000.exe C:\Users\价己酋\AppData\Local\Temp\TI94010 C:\Users\价己酋\AppData\Local\Temp\TI9404 
;	C:\step_tracer\a_3rd_race\Compiler\bin\ac2000.exe --keep_unneeded_types -D_INLINE -DLARGE_MODEL -IC:\step_tracer\a_3rd_race\include --version=28 --keep_unneeded_types --mem_model:code=flat --mem_model:data=large -m --i_output_file C:\Users\价己酋\AppData\Local\Temp\TI9402 --template_info_file C:\Users\价己酋\AppData\Local\Temp\TI9406 --object_file DSP280x_CpuTimers.obj --embed_opts 10 --call_assumptions=0 --mem_model:code=flat --mem_model:data=large --opt_for_speed --opt_level=3 --optimizer_comments 
	.sect	".text"
...
;*** 132	-----------------------    asm(" EDIS");
;*** 134	-----------------------    IER |= 0x2001u;
;*** 136	-----------------------    *(&PieCtrlRegs+2L) |= 0x40u;
;*** 137	-----------------------    *(&PieCtrlRegs+2L) |= 0x20u;
;*** 139	-----------------------    ConfigCpuTimer(&CpuTimer0, 100.0F, 25.0F);
;*** 140	-----------------------    ConfigCpuTimer(&CpuTimer2, 100.0F, 149.0F);
;*** 142	-----------------------    asm(" clrc INTM");
;*** 143	-----------------------    asm(" clrc DBGM");
;*** 143	-----------------------    return;
	.dwcfa	0x1d, -2
 ...
 EDIS
	...
	.dwpsn	"DSP280x_CpuTimers.c",136,2
        ...
        OR        @_PieCtrlRegs+2,#0x0040 ; |136| 
	.dwpsn	"DSP280x_CpuTimers.c",137,2
        OR        @_PieCtrlRegs+2,#0x0020 ; |137| 
	.dwpsn	"DSP280x_CpuTimers.c",139,2
        MOV       AL,#0
        MOV       AH,#16840
        MOVL      *-SP[2],ACC           ; |139| 
        SPM       #0
        MOVL      XAR4,#_CpuTimer0      ; |139| 
        MOV       AH,#17096
        LCR       #_ConfigCpuTimer      ; |139| 
        ; call occurs [#_ConfigCpuTimer] ; |139| 
	.dwpsn	"DSP280x_CpuTimers.c",140,2
        MOV       AL,#0
        MOV       AH,#17173
        MOVL      *-SP[2],ACC           ; |140| 
        MOVL      XAR4,#_CpuTimer2      ; |140| 
        MOV       AH,#17096
        LCR       #_ConfigCpuTimer      ; |140| 
 ...
 clrc DBGM
	...
	.dwattr DW$19, DW_AT_end_line(0x91)
