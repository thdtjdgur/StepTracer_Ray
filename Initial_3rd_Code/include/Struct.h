...
typedef volatile struct bit_field_flag
{
   ...
   ////////////////////////////////////////////////
   Uint16 third_turnmark_flag:1;
   Uint16 dist_check_flag:1;//시프트 해 준 직후 이동한 거리초기화를 위한 플래그 
...
typedef struct first_race_info
{
	...
	_iq7  iq7x_vel;
	////////////////////////////////////////////////
	volatile _iq10 iq10targetshift;
	volatile _iq10 iq10chop_targetshift;
	volatile _iq10 iq10chop_shift_before;//int32dist변수를 1000으로 나눈 값
	volatile _iq10 iq10chop_shift_after;//int32dist변수를 1000으로 나눈 값
	
	volatile _iq15 iq15chop_sdist;//int32dist변수를 1000으로 나눈 값

	volatile Uint16	DownFlag_U16:1;//3차주행 할지말지 결정플래그

	volatile _iq17	Kp_UpDown_IQ17;//각 주행 구간마다 적용되는 Kp의 “하한값 (최솟값)” 역할
}race_info;
