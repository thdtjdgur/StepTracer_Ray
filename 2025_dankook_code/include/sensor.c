...
void Handle( void )
{
	...
	{
		LMotor.iq15Handle = (_IQ17( 1.0 ) - _IQ17mpy( iq10position_real<<7, g_q17_handle_acc))>>2;//make_position함수에서 얻은 iq10temp_position와 
 		RMotor.iq15Handle = (_IQ17( 1.0 ) + _IQmpy( iq10position_real<<5, g_q17_handle_dec))>>2;//미리 정해둔 g_q17_handle_acc를 통해 iq15Handle이 값을 얻음 
...
{                                 //이 값을 바탕으로 REAL_MAXMIN함수가 진행되는 동안에도 ADC_TIMER_ISR인터럽트가 실행되기 때문에 실시간으로 127값이 정해지게 된다.
	...
	if( g_sen[g_int32_copmare_cnt  ].iq15_127_value >_IQ(60))//위에서 개산한 특정 adc핀의 127값이 iq(60)보다 크면 0000 0000 0000 0000 0000 
	{
	 ...
	 else if( g_Flag.cross == ON )//크로스를 방금 막 통과햇음 
	 {
		 ...
		 if( g_q15cross_dist > _IQ15( 150.0 ) )//???? cross_dist?? _IQ15( 150.0 )?? ?????? 
		 {
			 if((search_info[U16_turnmark_cnt].int32cross_check_dist == 0) && (g_Flag.first_race == ON)  )//1차 주행이고 다음 크로스를 만나면 그 이동만큼의 거리를 저장하는 내용임,int32cross_check_dist == 0인 이유는 다음 크로스를 만나 0으로 초기와됨// 한구간에 크로스가 여러개 잇을수 잇는데 그중 첫번재턴마크와 크로스가지의 거리저장
				 search_info[U16_turnmark_cnt].int32cross_check_dist = ((int32)(_IQ15mpy(RMotor.iq15GoneDist+LMotor.iq15GoneDist, _IQ15(0.5)))>>15) - 150;//int32cross_check_dist는 턴마크와 크로스 사이의 거리임//iq15GoneDist는 line_info에서 한족 턴마크 읽을대만 초기화됨.
		...
		//if(((g_sen[3].iq15_127_value>=_IQ(30))&&(g_sen[4].iq15_127_value>=_IQ(30))&&(g_sen[5].iq15_127_value<=_IQ(10))&&(g_sen[6].iq15_127_value<=_IQ(10))&&(g_sen[9].iq15_127_value<=_IQ(10))&&(g_sen[10].iq15_127_value<=_IQ(10))&&(g_sen[11].iq15_127_value>=_IQ(30))&&(g_sen[12].iq15_127_value>=_IQ(30))))
	  	{
			...
			g_u32_first_END_ACC_targetval = g_u32_first_END_ACC_targetval + end_accel;
		 	...
		 	{
			 	//move_to_end( _IQ(200.0), _IQ( LMotor.iq15NextV + RMotor.iq15NextV)>>1 , 8000 );//acc 8000, 브레이크
