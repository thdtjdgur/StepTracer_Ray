...
{	
	...
	if(g_Flag.third_turnmark_flag == OFF)//맨 처음에 off되어잇음
	{
		g_Flag.third_turnmark_flag = ON;// 이 조건문 다시안들어오게 on함 
	...
	if((search_info[U16_turnmark_cnt].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt+1].int32turn_dir >= TURN_45)&&(search_info[U16_turnmark_cnt+2].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt+1].int32turn_dir <= TURN_180))
	{
		...
		iq15third_dist_sum = ( LMotor.iq15third_dist + RMotor.iq15third_dist ) >> 1;//third_dist변수 개속 업데이트함
		...
		if((iq15third_dist_sum > search_info[U16_turnmark_cnt].chop_sdist)&&(g_Flag.third_straight_1 == ON))//실시간 거리인 third_dist변수가 미리 정해놓은 chop_sdist(전체 거리를 1000으로 나눈값)를 넘으면 
		{		 
			if((search_info[U16_turnmark_cnt+1].int32turn_dir & LEFT_TURN))//왼족 90도일대 
			{
				iq10position_shift -= chop_targetshift_90;
				g_Flag.third_turnmark_flag = OFF;//third_dist를 다시 초기화하기 위함 
				if(iq10position_shift < -iq10target_shift_90) iq10position_shift = -iq10target_shift_90;//에러처리
				return;
			}
			else//오른족 90도일대 
			{	
...
/*
	if((search_info[U16_turnmark_cnt-1].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt].int32turn_dir >= TURN_90)&&(search_info[U16_turnmark_cnt+1].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt].int32turn_dir <= TURN_180))
	{
		...
		int32third_dist_sum = ( RMotor.int32third_dist + LMotor.int32third_dist ) >> 1;//third_dist변수 개속 업데이트함

		if(_IQ15(int32third_dist_sum) > (search_info[U16_turnmark_cnt].chop_sdist))//실시간 거리인 third_dist변수가 미리 정해놓은 chop_sdist(전체 거리를 1000으로 나눈값)를 넘으면 
		{		 
			if((search_info[U16_turnmark_cnt].int32turn_dir & LEFT_TURN))//왼족 90도일대 
			{
				iq10position_shift += chop_targetshift_90;
				g_Flag.third_turnmark_flag = OFF;//third_dist를 다시 초기화하기 위함 
				if(iq10position_shift > _IQ10(0.0)) iq10position_shift = _IQ10(0.0);//에러처리
			}
			else//오른족 90도일대 
			{
				iq10position_shift -= chop_targetshift_90;
...
*/
	if((search_info[U16_turnmark_cnt-2].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt-1].int32turn_dir >= TURN_45)&&(search_info[U16_turnmark_cnt].int32turn_dir & STRAIGHT)&&(search_info[U16_turnmark_cnt-1].int32turn_dir <= TURN_180))
	{
		LMotor.iq15third_dist =  LMotor.iq15third_gone_dist;
		RMotor.iq15third_dist =  RMotor.iq15third_gone_dist;
		iq15third_dist_sum = ( RMotor.iq15third_dist + LMotor.iq15third_dist ) >> 1;//third_dist변수 개속 업데이트함

		if(iq15third_dist_sum > search_info[U16_turnmark_cnt].chop_sdist)//실시간 거리인 third_dist변수가 미리 정해놓은 chop_sdist(전체 거리를 1000으로 나눈값)를 넘으면 
		{		 
			if((search_info[U16_turnmark_cnt-1].int32turn_dir & LEFT_TURN))//왼족 90도일대 
			{
				//L_LED_ON;
				...
				{
					//L_LED_ON;//동방맵에서 75번재 턴마크의 직선에서 절반 이동하면 이 조건문에 들어와야하는데 거의 직선이 종료되기 20cm전에 여기 들어옴. 이유는 한 스텝당 0.824mm이동하고 이걸 1000번 햇다고 햇을대 
					//BUZZ_ON;//823mm임. 즉 인터럽트 1000번 도는게 목표(1000번 돌아야 내가 원하는 시프트에 도달할 수 잇음)인데 1000번 돌려면 트레이서가 823mm이동해야함. 근데 해당 직진의 길이는 1040mm임. 즉 인터럽트마다 시프트한다 해도 직선이 종료되기 20cm 전에 이 조건문에 들어올 수 잇는거임. 
							//트레이서가 이동하는 속도가 커서 모터인터럽트가 돌대마다 시프트 더하는 조건문에 들어옴. 그렇게 개속 더해도 더하는 양이 전체거리/1000박에 안더해져서 시프트속도가 느림. 
							//그래서 거리와 시프트를 1000으로 나누는게 아니라 500정도로 나누면 더해지는 시프트가 전체거리/500이니가 더 커져서 시프트속도가 커져서 내가 의도한 절반만 이동햇을대 이 조건문에 들어올 수 잇음  
					iq10position_shift = _IQ10(0.0);//에러처리
				}
				return;
			}
			else//오른족 90도일대 
	...
	*/
	// -3: 직선, -2: 90, -1: 직선 이고,
	//   (0이 90이 아니거나) || (0이 90인데 1이 직선이 아닐 때)
